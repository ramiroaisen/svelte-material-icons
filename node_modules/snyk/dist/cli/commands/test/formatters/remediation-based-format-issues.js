...
function thisUpgradeFixes(vulnIds, basicVulnInfo, testOptions) {
    return vulnIds
        ...
        .map((id) => formatIssue(id, basicVulnInfo[id].title, basicVulnInfo[id].severity, basicVulnInfo[id].isNew, `${basicVulnInfo[id].name}@${basicVulnInfo[id].version}`, basicVulnInfo[id].paths, testOptions, basicVulnInfo[id].note))
        .join('\n');
}
...
function constructUnfixableText(unresolved, basicVulnInfo, testOptions) {
    ...
    for (const issue of unresolved) {
        ...
        unfixableIssuesTextArray.push(formatIssue(issue.id, issue.title, issue.severity, issue.isNew, `${issue.packageName}@${issue.version}`, issueInfo.paths, testOptions, issueInfo.note) + `${extraInfo}`);
    }
...
function formatIssue(id, title, severity, isNew, vulnerableModule, paths, testOptions, note, legalInstructions) {
